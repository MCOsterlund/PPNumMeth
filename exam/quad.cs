...
public class integrate{
    ...
    public static double threequad(
        Func<double,double> f,
        double a,
        double b, 
        double delt=0.001,
        ...
        double h=b-a;
        if(double.IsNaN(f2)){f2=f(a+3*h/6);}
        double f1=f(a+h/6), f3=f(a+5*h/6);
        double Q=(3*f1+2*f2+3*f3)/8*(b-a);
        double q=(f1+f2+f3)/3*(b-a);
        double err=Abs(Q-q);
        if(err <= delt+eps*Abs(Q)) return Q;
        else return threequad(f,a,a+(b-a)/3,delt/Sqrt(2),eps, f1)+
                    threequad(f,a+(b-a)/3,a+2*(b-a)/3,delt/Sqrt(2),eps,f2)+
                    threequad(f,a+2*(b-a)/3,b,delt/Sqrt(2),eps,f3);
    }
}//integrate

//threequad(f,(a+b)/3,2*(a+b)/3,delt/Sqrt(2),eps,f2)+
//+
//;