...
class main{
    public static void Main(){
        WriteLine("Debugging some simple functions.\n");
    ...
    WriteLine("Investigating minima of Rosenbrock's Valley function: f(x,y)=(2-x)^2 + 100*(y-x^2)*x^2\n");

        Func<vector,vector> df=delegate(vector x){
            vector sol=new vector(1.0*((2-x[0])-100*(x[1]-x[0]*x[0])*x[0]),200*(x[1]-x[0]*x[0]));
            return sol;
        };
